...
- **Assessment domain consolidation** — the real stage-matched assessment is hosted at `assessment.catalystmomofficial.com` by its independently deployed Next.js app. Blog CTAs use `/postpartum-assessment`, `/pregnancy-assessment`, `/ttc-assessment`, or the assessment homepage for multi-stage content. The in-app `/questionnaire` and `/assessment-results` remain separate member features.
