...
function PostpartumGlowUpChallenge() {
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-6 mb-6 flex-wrap">
        {[
          ...
          <div key={label} className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full border-2 border-[#B5651D]/30 flex items-center justify-center">
              {icon}
            </div>
            <div>
      ...
      {/* Keyword tags — matches "Healing · Bone Broth · Lactation Support" */}
      <div className="flex gap-2 flex-wrap mb-6">
        {['Core Restore', 'Strength', 'Nutrition', 'Mindset', 'Recovery', 'Self-Care'].map(tag => (
          <span key={tag} className="border border-border text-muted-foreground rounded-full px-3 py-1 text-xs">
            {tag}
          </span>
        ))}
      </div>
      ...
      {activeTab === 'Daily Tasks' && (
        <div className="border border-border rounded-2xl overflow-hidden">
          <div className="grid md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-border">
            {/* Left: daily challenges — styled like Ingredients column */}
            <div className="p-6">
              ...
              <div className="space-y-5">
                {week.days.map(({ day, title, tasks, tip }) => (
                  <div key={day} className="flex gap-3 items-start border-b border-border pb-4 last:border-0 last:pb-0">
                    <div className="w-8 h-8 rounded-full bg-[#B5651D] text-white flex items-center justify-center text-xs font-bold shrink-0">
                      {day}
                    </div>
            ...
            {/* Right: week tips — styled like Instructions column */}
              ...
              <div className="space-y-4">
                <div className="rounded-xl bg-background border border-border p-4">
                  ...
                  <p className="text-sm text-muted-foreground leading-relaxed">
                    {week.subtitle} — {week.title.toLowerCase()}. Build the habit one day at a time.
                  </p>
                </div>
                <div className="rounded-xl bg-background border border-border p-4">
      ...
      {activeTab === 'Meal Plan' && (
        <div className="border border-border rounded-2xl overflow-hidden">
                ...
                {DAYS_OF_WEEK.map((day, i) => (
                  <tr key={day} className={i % 2 === 0 ? 'bg-background' : 'bg-muted/20'}>
                    <td className="px-4 py-3">
                      <span className="bg-[#B5651D] text-white text-xs font-bold px-2 py-1 rounded-md">{day}</span>
                    </td>
          ...
          <div className="p-4 bg-muted/30 border-t border-border">
            <p className="text-xs text-muted-foreground leading-relaxed">
              💡 <strong>Nutrition tip:</strong> This plan is a flexible guide — swap proteins and vegetables based on what's available.
              Focus on whole foods, adequate protein, and staying hydrated. Consult your healthcare provider for specific postpartum dietary needs.
            </p>
          </div>
        </div>
      )}
              ...
              <div className="space-y-4">
                {REFLECTION_QUESTIONS.map((q, i) => (
                  <div key={i} className="flex gap-3 items-start border-b border-border pb-4 last:border-0 last:pb-0">
                    <div className="w-6 h-6 rounded-full bg-[#B5651D] text-white flex items-center justify-center text-xs font-bold shrink-0 mt-0.5">
                      {i + 1}
                    </div>
...
const MealPlan = () => {
  ...
  // Always exclude postpartum plans from non-postpartum accounts
  const stageFilteredPlans = useMemo(() => {
    if (!journeyKey) {
      return mealPlans.filter((p: any) =>
        !Array.isArray(p.journey) || !p.journey.includes('postpartum')
      );
    }
    return mealPlans.filter((p: any) =>
      Array.isArray(p.journey) && p.journey.includes(journeyKey)
  ...
  useEffect(() => {
    if (isValid && journeyKey && recommended.length === 0 && journeyKey !== 'postpartum') {
      toast({
  ...
  // All other stages → their filtered meal plans only
  return (
    <PageLayout>
      <div className="container px-4 mx-auto py-6">
