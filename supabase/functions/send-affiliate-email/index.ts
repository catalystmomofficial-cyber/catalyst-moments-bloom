...
// Escape values interpolated into email HTML so a caller-supplied name/link
// can't inject markup.
const esc = (s: string) =>
  String(s ?? "")
    .replace(/&/g, "&amp;")
...
const applicationReceived = (name: string) => wrap(`
  <div style="background:#fff;border-radius:16px;padding:32px;border:1px solid #f0e4d6;">
    <h1 style="color:${BRAND_COPPER};font-size:26px;margin:0 0 12px;">Application received, ${esc(name)} 🌸</h1>
...
const applicationApproved = (name: string, link: string) => wrap(`
  <div style="background:#fff;border-radius:16px;padding:32px;border:1px solid #f0e4d6;">
    <h1 style="color:${BRAND_COPPER};font-size:26px;margin:0 0 12px;">You're in, ${esc(name)} 🎉</h1>
    <p style="font-size:15px;line-height:1.6;">Your affiliate account is active. Here's your unique referral link:</p>
    <div style="background:${BRAND_CREAM};border-radius:8px;padding:14px;margin:16px 0;word-break:break-all;font-weight:600;color:${BRAND_CHARCOAL};">${esc(link)}</div>
    <div style="text-align:center;margin:24px 0;">
      <a href="${esc(link.replace('/signup?ref=', '/affiliate/dashboard?welcome='))}" style="display:inline-block;background:${BRAND_COPPER};color:#fff;text-decoration:none;padding:14px 28px;border-radius:8px;font-weight:600;">Open Your Dashboard →</a>
    </div>
...
const payoutReady = (name: string, totalEarnings: string) => wrap(`
  <div style="background:#fff;border-radius:16px;padding:32px;border:1px solid #f0e4d6;">
    ...
    <p style="font-size:15px;line-height:1.6;">Hey ${esc(name)}, your referral has been confirmed — they completed their 2nd month with Catalyst Mom.</p>
    ...
    <div style="background:${BRAND_CREAM};border-radius:8px;padding:16px;margin:20px 0;text-align:center;">
      ...
      <div style="font-size:28px;font-weight:700;color:${BRAND_COPPER};">${esc(totalEarnings)}</div>
    </div>
